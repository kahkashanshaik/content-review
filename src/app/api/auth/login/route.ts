...
import { loginUser } from "@/server/auth/session";
...
export async function POST(request: Request): Promise<Response> {
  ...
  const result = await loginUser({ email, password }, stores.users, stores.sessions);
  if (!result.ok) {
    return jsonError(result.error);
  }

  const response = jsonOk({ user: result.value.user });
